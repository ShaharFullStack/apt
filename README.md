# מארז הדירות — שחר וענבל

אפליקציית אינטרנט פשוטה לדירוג דירות לשכירות. שני משתמשים קבועים (שחר וענבל) יכולים
להוסיף דירה, להעלות תמונות, למלא מחיר/ארנונה/ועד בית/איש קשר, ולתת ציון 1–10 לכל
קטגוריה (מיקום, מצב הדירה, אור וכו'). כל הנתונים נשמרים בקובץ SQLite מקומי בשרת —
בלי מסד נתונים חיצוני, בלי הרשמה, בלי ענן צד שלישי.
...
public/index.html  העמוד
public/styles.css  עיצוב
public/app.js      לוגיקת הצד לקוח
...
- שני המדרגים ("שחר" ו"ענבל") קבועים בקוד. אם תרצו לשנות שמות — מחפשים
  `const RATERS = ['שחר', 'ענבל'];` בתחילת `server.js`.
- קטגוריות הציונים ניתנות לעריכה ישירות באתר: לחיצה כפולה על שם קטגוריה
  לעריכה, כפתור "+ קטגוריה חדשה", וכפתור ✕ למחיקה.
- האתר לא כולל login/סיסמה — כל מי שיש לו את הכתובת יכול לערוך. אם השרת
  חשוף לאינטרנט הרחב ולא רק לכם, כדאי להוסיף הגנת סיסמה בסיסית ב-Nginx
  (Basic Auth) — אשמח לעזור להוסיף את זה אם תרצו.
